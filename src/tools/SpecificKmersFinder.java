...
public class SpecificKmersFinder extends Tool {
    ...
    protected void runImpl() throws ExecutionFailedException, IOException {
        ...
        long totalkmers = 0;
        long uniq = 0;
        long uniq_not_filtered_kmers = 0;
        long totalskippedchikmers = 0;
        long totalskippedMWkmers = 0;
        long totalkmersleft = 0;
        ...
        info("Group B k-mers printed to " + fileA.getPath());

        debug("Total kmers count = " + totalkmers);
        debug("Total unique kmers = " + uniq);
        debug("Total skipped by chi-squared test = " + totalskippedchikmers);
        debug("Total skipped by Mann-Whitney test = " + totalskippedMWkmers);
        debug("Total unique left = " + uniq_not_filtered_kmers);
    ...
    private boolean chisq(int c0, int c1, int p0, int p1, double value) {
        ...
        if (x1 != 0.0 && x2 != 0.0 && x3 != 0.0 && x4 != 0.0) {
            double kk = Math.pow(Math.abs(p1 - x1), 2) / x1 + Math.pow(Math.abs(p0 - x2), 2) / x2 + Math.pow(Math.abs(c1 - x3), 2) / x3 + Math.pow(Math.abs(c0 - x4), 2) / x4;
            return value < kk;
        }
        return false;
    }
